	         ...
	         GtkOrientation orientation)
{
  ...
  if (area)
    gdk_rectangle_intersect(&dest, area, &dest);
                   ...
                   gint gap_pos)
{
  ...
  if ((EDGE_LINE_STYLE(style,NULL) == SMOOTH_LINE_FLAT)  && DETAIL ("entry") && widget && (GTK_IS_SPIN_BUTTON (widget) || (is_combo_box_entry(widget)) || (is_combo(widget)))) 
  {
                 ...
                 GtkOrientation orientation)
{
  ...
  if (LINE_STYLE(style,NULL) == SMOOTH_LINE_FLAT) 
  {
      ...
      if (orientation==GTK_ORIENTATION_HORIZONTAL) 
      {
        gdk_draw_line(window, dark, start, base, end, base);      
      }
      else
      {
        gdk_draw_line(window, dark, base, start, base, end);      
      }  
...
{
  gint arrow_style;
  gboolean arrow_solid, arrow_etched;
  ...
  if (ETCHED_ARROW(style)) {
    GdkGC *light=NULL, *dark=NULL, *mid=NULL;

    dark = darktone_gc(style, state_type);
    light = lighttone_gc(style, state_type);
    mid = midtone_gc(style, state_type);

    do_draw_arrow(window, area, arrow_type, dark, light, mid, x, y, width, height, arrow_style, SOLID_ARROW(style), TRUE);
    ...
    GdkGC * fill_gc = style->fg_gc[state_type], *midaa_gc = NULL;
    GdkColor midaa;

    if (!solid)
          fill_gc = style->base_gc[state_type];

     midaa.red = (style->bg[state_type].red + style->fg[state_type].red) / 2;
     midaa.green = (style->bg[state_type].green + style->fg[state_type].green) / 2;
     midaa.blue = (style->bg[state_type].blue + style->fg[state_type].blue) / 2;

     midaa_gc = new_color_gc (style, &midaa);

     if ((arrow_style == ARROW_STYLE_WONDERLAND) && (!DETAIL("spinbutton")))
       do_draw_arrow(window, area, arrow_type, fill_gc, style->fg_gc[state_type], midaa_gc, x + 1, y + 1, width - 1, height - 1, arrow_style, solid, FALSE);
     else  
       do_draw_arrow(window, area, arrow_type, fill_gc, style->fg_gc[state_type], midaa_gc, x, y, width, height, arrow_style, solid, FALSE);
     
     gtk_gc_release(midaa_gc);
  }
}
                ...
                GtkOrientation orientation)
{
	...
	gboolean toolbar_overlap = (GRIP_OVERLAP_TOOLBAR(style) && (is_toolbar_item(widget)));
                   ...
                   gint height)
{
  ...
  if (THEME_PART(option)->edge.use_line) 
    line_style = &THEME_PART(option)->edge.line;
  else if (THEME_PART(option)->use_line) 
    ...
    if (((shadow_type == GTK_SHADOW_IN) || (!OPTION_MOTIF(style))) && (PART_STYLE(option) == NO_OPTION))
      ...
      smooth_draw_shadow_with_gap(style, window, state_type, OPTION_MOTIF(style)?shadow_type:GTK_SHADOW_IN, area, widget, "SMOOTH_DRAW_OPTION", THEME_PART(option), x, y, width, height, 0, 0, 0);
...
{  
  ...
  gint xthick = 0, ythick = 0;

  g_return_if_fail(sanitize_parameters(style, window, &width, &height));

  check = CHECK_PART(style);

  xthick = PART_XPADDING(check);
  ythick = PART_YPADDING(check);
  ...
  if (!((GTK_IS_CHECK_MENU_ITEM(widget) && (EDGE_LINE_STYLE(style, check) == SMOOTH_LINE_NONE)))) {
    ...
    if (CHECK_MOTIF(style)) 
      gradient_fill_background(style, window, state_type, area, widget, THEME_PART(check), x+xthick, y+ythick, width-xthick*2, height-ythick*2, FALSE, GTK_ORIENTATION_HORIZONTAL);
    else
      gdk_draw_rectangle(window, style->base_gc[state_type], TRUE, x+xthick, y+ythick, width-xthick*2, height-ythick*2);		     
  }


  if (shadow_type == GTK_SHADOW_IN)
  {
     xthick += EDGE_LINE_THICKNESS(style, check);
     ythick += EDGE_LINE_THICKNESS(style, check);

     if (EDGE_LINE_STYLE(style, check) == SMOOTH_LINE_COLD)
     {
	     xthick = 1 + PART_XPADDING(check);
	     ythick = 1 + PART_YPADDING(check);
     }

     draw_check_mark(window, style->text_gc[state_type], area, x+xthick, y+ythick, width-xthick*2, height-ythick*2, PART_STYLE(check));
  ...
  if (THEME_PART(check)->use_line || THEME_PART(check)->edge.use_line)
    smooth_draw_shadow_with_gap(style, window, state_type, shadow_type, area, widget, detail, THEME_PART(check), x+xthick, y+ythick, width-xthick*2, height-ythick*2, 0, 0, 0);
  else  
    gtk_paint_shadow(style, window, state_type, shadow_type, area, widget, detail, x+xthick, y+ythick, width-xthick*2, height-ythick*2);
}
